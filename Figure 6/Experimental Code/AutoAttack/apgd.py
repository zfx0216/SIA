...
class APGD(Attack):
    ...
    def attack_single_run(self, x_in, y_in):
        ...
        x_adv[0][0] = torch.clamp(x_adv[0][0], min=-2.1179, max=2.2489)
        x_adv[0][1] = torch.clamp(x_adv[0][1], min=-2.0357, max=2.4285)
        x_adv[0][2] = torch.clamp(x_adv[0][2], min=-1.8044, max=2.64)
        x_best = x_adv.clone()
        ...
        for i in range(self.steps):
            ...
            with torch.no_grad():
                ...
                if self.norm == "Linf":
                    ...
                    x_adv_1[0][0] = torch.clamp(torch.min(torch.max(x_adv_1[0][0], x[0][0] - self.eps), x[0][0] + self.eps), -2.1179, 2.2489)
                    x_adv_1[0][1] = torch.clamp(torch.min(torch.max(x_adv_1[0][1], x[0][1] - self.eps), x[0][1] + self.eps), -2.0357, 2.4285)
                    x_adv_1[0][2] = torch.clamp(torch.min(torch.max(x_adv_1[0][2], x[0][2] - self.eps), x[0][2] + self.eps), -1.8044, 2.64)

                    # x_adv_1 = torch.clamp(torch.min(torch.max(x_adv + (x_adv_1 - x_adv) * a + grad2 * (1 - a), x - self.eps), x + self.eps), -10.0, 10.0)
                    x_adv_1[0][0] = torch.clamp(torch.min(torch.max(x_adv[0][0] + (x_adv_1[0][0] - x_adv[0][0]) * a + grad2[0][0] * (1 - a), x[0][0] - self.eps), x[0][0] + self.eps), -2.1179, 2.2489)
                    x_adv_1[0][1] = torch.clamp(torch.min(torch.max(x_adv[0][1] + (x_adv_1[0][1] - x_adv[0][1]) * a + grad2[0][1] * (1 - a), x[0][1] - self.eps), x[0][1] + self.eps), -2.0357, 2.4285)
                    x_adv_1[0][2] = torch.clamp(torch.min(torch.max(x_adv[0][2] + (x_adv_1[0][2] - x_adv[0][2]) * a + grad2[0][2] * (1 - a), x[0][2] - self.eps), x[0][2] + self.eps), -1.8044, 2.64)
                    ...
                    #x_adv_1 = torch.clamp(x + (x_adv_1 - x) / (((x_adv_1 - x) ** 2).sum(dim=(1, 2, 3), keepdim=True).sqrt() + 1e-12) * torch.min(self.eps * torch.ones(x.shape).to(self.device).detach(), ((x_adv_1 - x) ** 2).sum(dim=(1, 2, 3), keepdim=True).sqrt()), -10.0, 10.0)  # nopep8


                    x_adv_1 = x_adv + (x_adv_1 - x_adv) * a + grad2 * (1 - a)
                    # x_adv_1 = torch.clamp(x + (x_adv_1 - x) / (((x_adv_1 - x) ** 2).sum(dim=(1, 2, 3), keepdim=True).sqrt() + 1e-12)
                    #     * torch.min(
                    ...
                    #     10.0,
                    # )  # nopep8
                    x_adv_1[0][0] = torch.clamp(x_adv_1[0][0], -2.1179, 2.2489)
                    x_adv_1[0][1] = torch.clamp(x_adv_1[0][1], -2.0357, 2.4285)
                    x_adv_1[0][2] = torch.clamp(x_adv_1[0][2], -1.8044, 2.64)
