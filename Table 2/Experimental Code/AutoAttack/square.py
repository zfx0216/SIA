...
class Square(Attack):
    ...
    def attack_single_run(self, x, y):
        with torch.no_grad():
            ...
            if self.norm == "Linf":
                x_best = torch.clamp(
                    x + self.eps * self.random_choice([x.shape[0], c, 1, w]), -10.0, 10.0
                )
                x_best[0][0] = torch.clamp(x_best[0][0], -2.1179, 2.2489)
                x_best[0][1] = torch.clamp(x_best[0][1], -2.0357, 2.4285)
                x_best[0][2] = torch.clamp(x_best[0][2], -1.8044, 2.64)
                ...
                for i_iter in range(self.n_queries):
                    ...
                    x_new[0][0] = torch.clamp(x_new[0][0], -2.1179, 2.2489)
                    x_new[0][1] = torch.clamp(x_new[0][1], -2.0357, 2.4285)
                    x_new[0][2] = torch.clamp(x_new[0][2], -1.8044, 2.64)
                ...
                x_best = torch.clamp(
                    x + self.normalize_delta(delta_init) * self.eps, -10.0, 10.0
                )
                x_best[0][0] = torch.clamp(x_best[0][0], -2.1179, 2.2489)
                x_best[0][1] = torch.clamp(x_best[0][1], -2.0357, 2.4285)
                x_best[0][2] = torch.clamp(x_best[0][2], -1.8044, 2.64)
                ...
                for i_iter in range(self.n_queries):
                    ...
                    x_new = torch.clamp(
                        x_curr + self.normalize_delta(delta_curr) * self.eps, -10.0, 10.0
                    )
                    x_new[0][0] = torch.clamp(x_new[0][0], -2.1179, 2.2489)
                    x_new[0][1] = torch.clamp(x_new[0][1], -2.0357, 2.4285)
                    x_new[0][2] = torch.clamp(x_new[0][2], -1.8044, 2.64)
